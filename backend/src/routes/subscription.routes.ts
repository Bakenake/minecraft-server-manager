...
export default async function subscriptionRoutes(fastify: FastifyInstance) {
  ...
  // ─── Generate a permanent premium license key (admin) ─────
  fastify.post(
    '/api/subscription/generate-key',
    async (
      ...
      if (user?.role !== 'admin') {
        return reply.code(403).send({ error: 'Only administrators can generate license keys.' });
      }

      const { email = '', note = '', duration = 'lifetime' } = request.body || {};
      ...
      await db.insert(licensesTable).values({
        ...
        tier: 'premium',
        status: 'active',
        email: email || 'manual@craftos.app',
        hardwareId: null, // unbound — anyone can redeem it
        activatedAt: null,
        expiresAt,
        maxServers: PREMIUM_TIER.maxServers,
        maxRamMb: PREMIUM_TIER.maxRamMb,
      ...
      audit({
        ...
        action: 'license.key_generated',
        resource: 'subscription',
        details: { licenseKey, email, note, duration, expiresAt: expiresAt?.toISOString() ?? null },
        ipAddress: request.ip,
      });

      fastify.log.info({ licenseKey, email, duration, note }, 'Premium key generated by admin');
      ...
      return reply.send({
        ...
        message: `Premium key generated successfully (${duration}).`,
      });
    },
  );

  // ─── List all generated license keys (admin) ──────────────
  fastify.get(
    '/api/subscription/keys',
    async (request: FastifyRequest, reply: FastifyReply) => {
      const user = (request as any).user;
      if (user?.role !== 'admin') {
        return reply.code(403).send({ error: 'Only administrators can view license keys.' });
      }

      const db = getDb();
      const allKeys = await db.select().from(licensesTable).orderBy(licensesTable.createdAt);
      ...
      const keys = allKeys.map((k) => ({
        ...
        status: k.status,
        email: k.email,
        hardwareId: k.hardwareId ? `${k.hardwareId.substring(0, 8)}...` : null,
        isBound: !!k.hardwareId,
  ...
  fastify.post(
    '/api/subscription/keys/:id/revoke',
    async (
      ...
      const user = (request as any).user;
      if (user?.role !== 'admin') {
        return reply.code(403).send({ error: 'Only administrators can revoke keys.' });
      }
      ...
      if (existing.length === 0) {
        return reply.code(404).send({ error: 'License key not found.' });
      }

      const key = existing[0];
      if (key.status === 'revoked') {
        return reply.code(400).send({ error: 'Key is already revoked.' });
      }
      ...
      // Also deactivate any activations tied to this key
      await db
      ...
      audit({
        ...
        resource: 'subscription',
        details: { licenseId: id, licenseKey: key.licenseKey },
        ipAddress: request.ip,
      });

      fastify.log.info({ id, licenseKey: key.licenseKey }, 'License key revoked by admin');

      return reply.send({
        success: true,
        message: `Key ${key.licenseKey} has been revoked.`,
      });
    },
  );
}
